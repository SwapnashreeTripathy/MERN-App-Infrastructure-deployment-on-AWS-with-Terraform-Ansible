...
resource "aws_route_table" "private_RT_Natg1a" {
    vpc_id = var.vpc_id
    ...
    tags = {
      Name = "private_RT_Natg1a"
    }
  
}


#Create 2nd route table for nat-gateway-1b
resource "aws_route_table" "private_RT_Natg1b" {
    vpc_id = var.vpc_id
    ...
    tags = {
      Name = "private_RT_Natg1b"
    }
  
}

# associate private subnet pri-sub-1st with private route table private_RT_Natg1a
resource "aws_route_table_association" "pri_sub_1st_with_private_RT_Natg1a" {
  subnet_id         = var.pri_sub_1st_id
  route_table_id    = aws_route_table.private_RT_Natg1a.id
}

# associate private subnet pri-sub-2nd with private route table private_RT_Natg1b
resource "aws_route_table_association" "pri_sub_2nd_with_private_RT_Natg1b" {
  subnet_id         = var.pri_sub_2nd_id
  route_table_id    = aws_route_table.private_RT_Natg1b.id
}
